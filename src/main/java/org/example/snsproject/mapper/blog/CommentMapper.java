...
public interface CommentMapper {
    ...
    @Select("select * from comment_article join user on comment_article.author_id = user.id where parent_id IS NULL and article_id=#{id} order by create_date desc")
    @Results({
    ...
    @Select("select * from comment_article join user on comment_article.author_id = user.id where parent_id=#{parentId} order by create_date")
    @Results({
            @Result(property = "id", column = "id"),
            @Result(property = "content", column = "content"),
            @Result(property = "createDate", column = "create_date"),
    ...
    @Insert("INSERT into comment_article(content, create_date, article_id, author_id, parent_id, to_user, level) " +
            "values(#{content}, #{createDate}, #{articleId}, #{authorId}, #{parentId}, #{toUid}, #{level})")
    void createComment(Comment comment);

    @Insert("INSERT into comment_article(content, create_date, article_id, author_id, level) " +
